...
FGFS_BASE_CMD = "pycasx launch background=false httpd=null telnet=null disable_sound=false aircraft=null altitude=null heading=null lat=null lon=null pitch=null roll=null vc=null config=null prop=null timeofday=null wind=null ai_scenario=null"  # noqa: E501
PYCASX_BASE_CMD = (
    "pycasx acasx logger.log_folder=logs/{uuids[_idx]}/{run_} logger.include_date=false"
)
...
def run(cfg: RunConfig) -> None:
    ...
    for _idx, scenario in enumerate(tqdm(scenarios)):
        for run_ in range(3):
            cmd_ = eval(f'f"{PYCASX_BASE_CMD}"')  # noqa: S307
            if run_ == 0:
