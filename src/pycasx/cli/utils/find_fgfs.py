...
def find_fgfs(with_root: bool = False) -> Path | tuple[Path, Path]:
    ...
    Args:
        with_root (bool): If true, return the root data directory as
            well.

    Returns:
        Path | tuple[Path, Path]: The fully resolved path to the
            FlightGear executable and the root data directory if
            `with_root` is true.
