...
def launch(cfg: LaunchConfig) -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    # Under Windows, FlightGear sometimes fails to find the data
    # directory.
    ...
    if cfg.fgfsrc is not None:
        with Path(cfg.fgfsrc).open(encoding="utf-8") as f:
            line = f.readline()
            while line:
                if line[0] != "#":
                    command += line.split()
