...
class Runner:
    ...
    def _hcas_autoavoid(self) -> None:
        """Execute the HCAS autoavoid action."""
        self.action = HCAS_ACTIONS[self.hcas_adv]  # type: ignore
    ...
    def _vcas_autoavoid(self) -> None:
        """Execute the VCAS autoavoid action."""
        self.action = VCAS_ACTIONS[self.vcas_adv]  # type: ignore
        self.command = self.action if not isinstance(self.action, str) else None
    ...
    def log(self) -> None:
        ...
        for topic, file in self.logger_cfg.files.items():
            try:
                ...
                with Path(str(Path(self.log_folder, file).resolve())).open(
                    encoding="utf-8"
                ) as f:
                    lines = f.readlines()
                lines = lines[:-1]
                lines.append(json.dumps(resp, indent=2) + ",\n]")
                Path(self.log_folder, file).open("w", encoding="utf-8").writelines(  # noqa: SIM115
                    lines
                )
    ...
    def init_log(self) -> None:
        ...
        for file in self.logger_cfg.files.values():
    ...
    def print_status(self) -> None:
        ...
        def format_title(title: str, max_width: int) -> str:
        ...
        def format_line(label: str, value: Any, max_width: int) -> str:
            return (
    ...
    def get_intruders(self) -> dict[str, ExtendedIntruder | float]:
        ...
        if not self.acasx.intruders:
            return {}
        intruders: dict[str, ExtendedIntruder | float] = {}
