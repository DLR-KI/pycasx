...
class ACASX(HCASMixin, VCASMixin):
    ...
    Args:
        ...
        backend (Literal["nnet", "onnx", "torch"], optional): The
            runtime backend to use. Defaults to "nnet".
    """
    ...
    def advise(self) -> tuple[HCASAdvisories, VCASAdvisories]:  # noqa: C901
        ...
        for intruder in self.intruders.values():
            ...
            if len(self.intruders) < 2:  # noqa: PLR2004
    ...
    def fetch_data(self) -> None:  # noqa: PLR0914
        ...
        for model in aircraft:
            ...
            # Errico, A. and Di Vito, V. (2019): Methodology for
            # Estimation of Closest Point of Approach between Aircraft
            # in ATM
            #
            # Ensure all units are in SI and all variable are magnitude
            # only
            v_own = self.ownship.true_airspeed.to(ureg.meter / ureg.second).magnitude
            v_int = ac_int.true_airspeed.to(ureg.meter / ureg.second).magnitude
            rho = dist.to(ureg.meter).magnitude
            u = np.array([
                v_own * math.sin(self.ownship.heading.to(ureg.radian).magnitude),
                v_own * math.cos(self.ownship.heading.to(ureg.radian).magnitude),
            ])
            v = np.array([
                v_int * math.sin(ac_int.heading.to(ureg.radian).magnitude),
                v_int * math.cos(ac_int.heading.to(ureg.radian).magnitude),
            ])
            w_0 = np.array([
    ...
    def fetch_intruder_data(
        ...
        Returns:
            Aircraft: The intruder with the data fetched from
                FlightGear.
        """
        return self.fetch_aircraft_data(
    ...
    def fetch_aircraft_data(  # noqa: PLR0913, PLR0917
        self,
        call_sign: str,
        altitude_ft: str,
        ...
        Returns:
            Aircraft: The aircraft with the data fetched from
                FlightGear.
